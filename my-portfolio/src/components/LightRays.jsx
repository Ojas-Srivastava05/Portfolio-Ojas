...
/**
 * Volumetric god-rays background (spell.sh "Light Rays", three.js shader).
 * Adapted to JSX + defaults to a transparent background so it can overlay
 * a video/aurora. Recolored to the portfolio's emerald/amber palette.
...
export default function LightRays({
  ...
  animation = { animate: true, speed: 7 },
  raysColor = { mode: "multi", color1: "#34d399", color2: "#fbbf24" },
  style,
  className,
}) {
  ...
  useEffect(() => {
    animationRef.current = { ...animation, animate: animation.animate && !reduceMotion };
  ...
  useEffect(() => {
    ...
    if (!container || !isMounted) return undefined;
    ...
    const handleResize = () => {
      if (!container) return;
      renderer.setSize(container.clientWidth, container.clientHeight);
      material.uniforms.u_resolution.value = [container.clientWidth, container.clientHeight];
  ...
  }, [intensity, rays, reach, color1RGB, color2RGB]);
